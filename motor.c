...
void aumentaMedia(float *trajeto, int tamanho)
{
    ...
    {
        linha = i * 3;
        maior = *(trajeto + linha + 1) > maior ? *(trajeto + linha + 1) : maior;
    }
    ...
    {
        linha = i * 3;
        menor = (*(trajeto + linha + 1) < menor) && *(trajeto + linha + 1) != 0 ? *(trajeto + linha + 1) : menor;
    }

    for (int i = 0; i < tamanho; i++)
    {
        linha = i * 3;
        if (*(trajeto + linha + 1) == menor)
        {
            *(trajeto + linha + 1) += 1;
        }
    }
